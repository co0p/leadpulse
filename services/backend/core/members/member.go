...
// Reactivate marks the team member as active again.
// Can only reactivate a deactivated member; returns error if already active.
func (tm *TeamMember) Reactivate() error {
	if tm.IsActive() {
		return fmt.Errorf("team member is already active")
	}
	tm.deactivatedAt = nil
	return nil
}
